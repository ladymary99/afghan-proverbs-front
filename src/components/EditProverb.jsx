...
const EditProverb = ({ proverb, onProverbSaved, onCancel }) => {
  ...
  useEffect(() => {
    if (proverb) {
      setFormData({
        textDari: proverb.textDari || "",
        textPashto: proverb.textPashto || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await proverbsAPI.updateProverb(proverb.id, formData);
      onProverbSaved();
    } catch (error) {
      console.error("Error updating proverb:", error);
      setErrors({ submit: "Failed to update proverb. Please try again." });
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!proverb) {
    return <div className="error">No proverb selected for editing.</div>;
  }

  return (
    <div className="edit-proverb">
      <div className="form-header">
        <h2>Edit Proverb</h2>
        <p>Update the details of this proverb</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="proverb-form">
        ...
        <div className="form-actions">
          ...
          <button type="submit" className="submit-button" disabled={loading}>
            {loading ? "Updating..." : "Update Proverb"}
          </button>
        </div>
      </form>
    </div>
